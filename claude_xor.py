...
output_folder = 'byte_xor'

# Create the output folder if it doesn't exist
os.makedirs(output_folder, exist_ok=True)

# Function to XOR two hex strings byte-wise
def xor_hex_bytes(hex1, hex2):
    result = []
    for i in range(0, len(hex1), 2):
        byte1 = int(hex1[i:i+2], 16)
        byte2 = int(hex2[i:i+2], 16)
        xor_bytes = byte1 ^ byte2
        result.append('{:02X}'.format(xor_bytes))
    return ''.join(result)
...
if file_a and file_b:
    ...
    if len(hex1) != len(hex2):
        ...
        # XOR the hex strings byte-wise
        result = xor_hex_bytes(hex1, hex2)
