...
output_folder = 'byte_add'

# Create the output folder if it doesn't exist
os.makedirs(output_folder, exist_ok=True)

# Function to add two hex strings byte-wise with modulo 256
def add_hex_bytes(hex1, hex2):
    result = []
    for i in range(0, len(hex1), 2):
        byte1 = int(hex1[i:i+2], 16)
        byte2 = int(hex2[i:i+2], 16)
        sum_bytes = (byte1 + byte2) % 256
        result.append('{:02X}'.format(sum_bytes))
    return ''.join(result)
...
if file_a and file_b:
    ...
    if len(hex1) != len(hex2):
        ...
        # Add the hex strings byte-wise
        result = add_hex_bytes(hex1, hex2)
