...
output_folder = 'nibble_add'

# Create the output folder if it doesn't exist
os.makedirs(output_folder, exist_ok=True)

# Function to add two hex strings nibble-wise with modulo 16 and no carry
def add_hex_nibbles(hex1, hex2):
    result = []
    for i in range(len(hex1)):
        nibble1 = int(hex1[i], 16)
        nibble2 = int(hex2[i], 16)
        sum_nibbles = (nibble1 + nibble2) % 16
        result.append('{:X}'.format(sum_nibbles))
    return ''.join(result)
...
if file_a and file_b:
    ...
    if len(hex1) != len(hex2):
        print("Input files have different lengths. Please ensure they are both of equal length.")
    else:
        # Add the hex strings nibble-wise
        result = add_hex_nibbles(hex1, hex2)
