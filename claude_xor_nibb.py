...
output_folder = 'nibble_xor'

# Create the output folder if it doesn't exist
os.makedirs(output_folder, exist_ok=True)

# Function to XOR two hex strings nibble-wise
def xor_hex_nibbles(hex1, hex2):
    result = []
    for i in range(len(hex1)):
        nibble1 = int(hex1[i], 16)
        nibble2 = int(hex2[i], 16)
        xor_nibbles = nibble1 ^ nibble2
        result.append('{:X}'.format(xor_nibbles))
    return ''.join(result)
...
if file_a and file_b:
    ...
    if len(hex1) != len(hex2):
        ...
        # XOR the hex strings nibble-wise
        result = xor_hex_nibbles(hex1, hex2)
